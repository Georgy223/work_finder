...
$query = "SELECT `vacancy_title`, `vacancy_salary`, `vacancy_currency`, `vacancy_schedule`, `vacancy_experience`, `vacancy_description`, 
       `user_company`
FROM `vacancies` 
JOIN users ON vacancy_user=user_id";
$where = "";
if (checkPostVar($title)) {
    $where .= "vacancy_title LIKE '%$title%' ";
}
if (checkPostVar($min) && checkPostVar($max)) {
    $where .= "AND vacancy_salary >= {$min} AND vacancy_salary <= {$max} ";
}
if (checkPostVar($currency)) {
    $where .= "AND vacancy_currency='{$currency}' ";
}
if (checkPostVar($schedule)) {
    $where .= "AND vacancy_schedule in {$schedule} ";
}
if (checkPostVar($experience)) {
    $where .= "AND vacancy_experience > {$experience} ";
}
...
if (isset($result) && $result != false)
{
    while($row = mysqli_fetch_row($result))
    {
        ...
        $cards .= getVacancy($title,$salary,$currency,$schedule,$experience,$description, $company);
    }
}
...
<div class="form-container container d-flex align-items-center justify-content-center w-50">
    <form class="bg-white p-3" action="vacancies.php" method="post">
        <h2>Вакансии</h2>
        <div class="row">
            <div class="mt-3 col">
